...
void x86_instruction::encode()
{
	...
	this->decode(buf, olen, get_machine_mode(), get_stack_addr_width());
}

xed_machine_mode_enum_t x86_instruction::get_machine_mode() const
{
	xed_machine_mode_enum_t mmode;
	switch (this->get_machine_mode_bits())
	{
		case 16:
		{
...
xed_address_width_enum_t x86_instruction::get_stack_addr_width() const
{
	xed_address_width_enum_t stack_addr_width;
	switch (this->get_stack_address_mode_bits())
	{
		case 16: stack_addr_width = XED_ADDRESS_WIDTH_16b; break;
		case 32: stack_addr_width = XED_ADDRESS_WIDTH_32b; break;
		case 64: stack_addr_width = XED_ADDRESS_WIDTH_64b; break;
		default:
		{
...
std::vector<xed_uint8_t> x86_instruction::get_bytes() const
{
	...
	const xed_uint_t len = this->get_length();
...
void x86_instruction::get_read_written_registers(std::vector<x86_register>* read_registers, std::vector<x86_register>* written_registers) const
{
	...
	for (const x86_operand& operand : operands)
	{
		...
		if (targetReg && !targetReg.is_pseudo())
		{
			if (hasRead)
...
std::string x86_instruction::to_string() const
{
	char buf[64];
	this->sprintf(buf, 64);
	return buf;
...
{
	xed_bool_t result = xed_format_context(XED_SYNTAX_INTEL, this, buf, length, this->m_addr, 0, 0);
	if (result == 0)
	{
		// failed
		throw std::runtime_error("xed_format_context failed");
	}
}

std::ostream& operator<<(std::ostream& stream, const x86_instruction& instruction)
{
	return stream << instruction.to_string();
}
std::ostream& operator<<(std::ostream& stream, const x86_instruction* instruction)
{
	return stream << instruction->to_string();
