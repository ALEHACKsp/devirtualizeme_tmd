...
public:
	...
	xed_machine_mode_enum_t get_machine_mode() const;
	xed_address_width_enum_t get_stack_addr_width() const;
	...
	inline unsigned int read_flag_set_mask() const
	{
		const xed_simple_flag_t* rfi = xed_decoded_inst_get_rflags_info(this);
		if (!rfi)
			return 0;
		return xed_flag_set_mask(xed_simple_flag_get_read_flag_set(rfi));
	}
